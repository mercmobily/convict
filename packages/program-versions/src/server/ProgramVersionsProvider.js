...
const CRUD_MODULE_CONFIG = Object.freeze({
  namespace: "program_versions",
  surface: "app",
  ownershipFilter: "public",
  relativePath: "/program-versions"
});
...
function resolveCrudPolicyFromApp(app) {
  return resolveCrudSurfacePolicyFromAppConfig(CRUD_MODULE_CONFIG, resolveAppConfig(app), {
    context: "ProgramVersionsProvider"
  });
}

class ProgramVersionsProvider {
  static id = "crud.program_versions";
  ...
  register(app) {
    ...
    app.singleton("repository.program_versions", (scope) => {
    ...
    app.service(
      ...
      (scope) => {
        return createService({
          programVersionsRepository: scope.make("repository.program_versions")
        });
      },
      {
    ...
    app.actions(
      withActionDefaults(
        ...
        }),
        {
          domain: "crud",
          dependencies: {
            programVersionsService: "crud.program_versions"
          }
        }
      )
    );
...
export { ProgramVersionsProvider };
