...
const CRUD_MODULE_CONFIG = Object.freeze({
  ...
  relativePath: "/user-progression-track-progress"
});
...
function resolveCrudPolicyFromApp(app) {
  return resolveCrudSurfacePolicyFromAppConfig(CRUD_MODULE_CONFIG, resolveAppConfig(app), {
    context: "UserProgressionTrackProgressProvider"
  });
}

class UserProgressionTrackProgressProvider {
  static id = "crud.user_progression_track_progress";
  ...
  register(app) {
    ...
    app.singleton("repository.user_progression_track_progress", (scope) => {
    ...
    app.actions(
      withActionDefaults(
        ...
        }),
        {
          domain: "crud",
          dependencies: {
            userProgressionTrackProgressService: "crud.user_progression_track_progress"
          }
        }
      )
    );
...
export { UserProgressionTrackProgressProvider };
