...
const CRUD_MODULE_CONFIG = Object.freeze({
  namespace: "exercise_steps",
  surface: "app",
  ownershipFilter: "public",
  relativePath: "/exercise-steps"
});
...
function resolveCrudPolicyFromApp(app) {
  return resolveCrudSurfacePolicyFromAppConfig(CRUD_MODULE_CONFIG, resolveAppConfig(app), {
    context: "ExerciseStepsProvider"
  });
}

class ExerciseStepsProvider {
  static id = "crud.exercise_steps";
  ...
  register(app) {
    ...
    app.singleton("repository.exercise_steps", (scope) => {
    ...
    app.service(
      ...
      (scope) => {
        return createService({
          exerciseStepsRepository: scope.make("repository.exercise_steps")
        });
      },
      {
    ...
    app.actions(
      withActionDefaults(
        ...
        }),
        {
          domain: "crud",
          dependencies: {
            exerciseStepsService: "crud.exercise_steps"
          }
        }
      )
    );
