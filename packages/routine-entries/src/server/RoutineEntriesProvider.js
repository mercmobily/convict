...
function resolveCrudPolicyFromApp(app) {
  return resolveCrudSurfacePolicyFromAppConfig(CRUD_MODULE_CONFIG, resolveAppConfig(app), {
    context: "RoutineEntriesProvider"
  });
}

class RoutineEntriesProvider {
  static id = "crud.routine_entries";
  ...
  register(app) {
    ...
    app.singleton("repository.routine_entries", (scope) => {
    ...
    app.service(
      ...
      (scope) => {
        return createService({
          routineEntriesRepository: scope.make("repository.routine_entries")
        });
      },
      {
    ...
    app.actions(
      withActionDefaults(
        ...
        }),
        {
          domain: "crud",
          dependencies: {
            routineEntriesService: "crud.routine_entries"
          }
        }
      )
    );
