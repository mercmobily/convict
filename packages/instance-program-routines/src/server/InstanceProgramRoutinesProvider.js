...
const CRUD_MODULE_CONFIG = Object.freeze({
  namespace: "instance_program_routines",
  surface: "app",
  ownershipFilter: "user",
  relativePath: "/instance-program-routines"
});
...
function resolveCrudPolicyFromApp(app) {
  return resolveCrudSurfacePolicyFromAppConfig(CRUD_MODULE_CONFIG, resolveAppConfig(app), {
    context: "InstanceProgramRoutinesProvider"
  });
}

class InstanceProgramRoutinesProvider {
  static id = "crud.instance_program_routines";
  ...
  register(app) {
    ...
    app.singleton("repository.instance_program_routines", (scope) => {
    ...
    app.actions(
      withActionDefaults(
        ...
        }),
        {
          domain: "crud",
          dependencies: {
            instanceProgramRoutinesService: "crud.instance_program_routines"
          }
        }
      )
    );
...
export { InstanceProgramRoutinesProvider };
