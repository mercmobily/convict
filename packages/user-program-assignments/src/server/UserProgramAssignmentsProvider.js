...
const CRUD_MODULE_CONFIG = Object.freeze({
  ...
  relativePath: "/user-program-assignments"
});
...
function resolveCrudPolicyFromApp(app) {
  return resolveCrudSurfacePolicyFromAppConfig(CRUD_MODULE_CONFIG, resolveAppConfig(app), {
    context: "UserProgramAssignmentsProvider"
  });
}

class UserProgramAssignmentsProvider {
  static id = "crud.user_program_assignments";
  ...
  register(app) {
    ...
    app.singleton("repository.user_program_assignments", (scope) => {
    ...
    app.actions(
      withActionDefaults(
        ...
        }),
        {
          domain: "crud",
          dependencies: {
            userProgramAssignmentsService: "crud.user_program_assignments"
          }
        }
      )
    );
...
export { UserProgramAssignmentsProvider };
