...
function resolveCrudPolicyFromApp(app) {
  return resolveCrudSurfacePolicyFromAppConfig(CRUD_MODULE_CONFIG, resolveAppConfig(app), {
    context: "ProgramRoutinesProvider"
  });
}

class ProgramRoutinesProvider {
  static id = "crud.program_routines";
  ...
  register(app) {
    ...
    app.singleton("repository.program_routines", (scope) => {
    ...
    app.actions(
      withActionDefaults(
        ...
        }),
        {
          domain: "crud",
          dependencies: {
            programRoutinesService: "crud.program_routines"
          }
        }
      )
    );
...
export { ProgramRoutinesProvider };
