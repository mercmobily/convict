...
function resolveCrudPolicyFromApp(app) {
  return resolveCrudSurfacePolicyFromAppConfig(CRUD_MODULE_CONFIG, resolveAppConfig(app), {
    context: "ProgramEntriesProvider"
  });
}

class ProgramEntriesProvider {
  static id = "crud.program_entries";
  ...
  register(app) {
    ...
    app.singleton("repository.program_entries", (scope) => {
    ...
    app.service(
      ...
      (scope) => {
        return createService({
          programEntriesRepository: scope.make("repository.program_entries")
        });
      },
      {
    ...
    app.actions(
      withActionDefaults(
        ...
        }),
        {
          domain: "crud",
          dependencies: {
            programEntriesService: "crud.program_entries"
          }
        }
      )
    );
