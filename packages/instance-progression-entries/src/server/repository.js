...
const JSON_REST_SCOPE_NAME = "instanceProgressionEntries";

function createRepository({ api, knex } = {}) {
  const withTransaction = createWithTransaction(knex);

  async function queryDocuments(query = {}, options = {}) {
    return api.resources.instanceProgressionEntries.query(
      {
  ...
  async function getDocumentById(recordId, options = {}) {
    return returnNullWhenJsonRestResourceMissing(() =>
      api.resources.instanceProgressionEntries.get(
        {
          id: recordId,
  ...
  async function createDocument(payload = {}, options = {}) {
    return api.resources.instanceProgressionEntries.post(
      {
  ...
  async function patchDocumentById(recordId, patch = {}, options = {}) {
    ...
    return returnNullWhenJsonRestResourceMissing(() =>
      api.resources.instanceProgressionEntries.patch(
        {
          id: recordId,
  ...
  async function deleteDocumentById(recordId, options = {}) {
    return returnNullWhenJsonRestResourceMissing(async () => {
      await api.resources.instanceProgressionEntries.delete(
        {
          id: recordId,
          transaction: options?.trx || null,
          simplified: false
