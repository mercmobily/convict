...
const CRUD_MODULE_CONFIG = Object.freeze({
  ...
  relativePath: "/instance-progression-entries"
});
...
function resolveCrudPolicyFromApp(app) {
  return resolveCrudSurfacePolicyFromAppConfig(CRUD_MODULE_CONFIG, resolveAppConfig(app), {
    context: "InstanceProgressionEntriesProvider"
  });
}

class InstanceProgressionEntriesProvider {
  static id = "crud.instance_progression_entries";
  ...
  register(app) {
    ...
    app.singleton("repository.instance_progression_entries", (scope) => {
    ...
    app.actions(
      withActionDefaults(
        ...
        }),
        {
          domain: "crud",
          dependencies: {
            instanceProgressionEntriesService: "crud.instance_progression_entries"
          }
        }
      )
    );
...
export { InstanceProgressionEntriesProvider };
