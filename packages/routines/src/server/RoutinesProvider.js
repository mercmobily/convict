...
function resolveCrudPolicyFromApp(app) {
  return resolveCrudSurfacePolicyFromAppConfig(CRUD_MODULE_CONFIG, resolveAppConfig(app), {
    context: "RoutinesProvider"
  });
}

class RoutinesProvider {
  static id = "crud.routines";
  ...
  register(app) {
    ...
    app.singleton("repository.routines", (scope) => {
    ...
    app.service(
      ...
      (scope) => {
        return createService({
          routinesRepository: scope.make("repository.routines")
        });
      },
      {
