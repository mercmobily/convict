...
const CRUD_MODULE_CONFIG = Object.freeze({
  namespace: "program_assignments",
  surface: "app",
  ownershipFilter: "user",
  relativePath: "/program-assignments"
});
...
function resolveCrudPolicyFromApp(app) {
  return resolveCrudSurfacePolicyFromAppConfig(CRUD_MODULE_CONFIG, resolveAppConfig(app), {
    context: "ProgramAssignmentsProvider"
  });
}

class ProgramAssignmentsProvider {
  static id = "crud.program_assignments";
  ...
  register(app) {
    ...
    app.singleton("repository.program_assignments", (scope) => {
    ...
    app.actions(
      withActionDefaults(
        ...
        }),
        {
          domain: "crud",
          dependencies: {
            programAssignmentsService: "crud.program_assignments"
          }
        }
      )
    );
