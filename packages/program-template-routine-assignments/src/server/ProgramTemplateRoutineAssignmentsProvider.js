...
const CRUD_MODULE_CONFIG = Object.freeze({
  namespace: "program_template_routine_assignments",
  surface: "app",
  ownershipFilter: "public",
  relativePath: "/program-template-routine-assignments"
});
...
function resolveCrudPolicyFromApp(app) {
  return resolveCrudSurfacePolicyFromAppConfig(CRUD_MODULE_CONFIG, resolveAppConfig(app), {
    context: "ProgramTemplateRoutineAssignmentsProvider"
  });
}

class ProgramTemplateRoutineAssignmentsProvider {
  static id = "crud.program_template_routine_assignments";
  ...
  register(app) {
    ...
    app.singleton("repository.program_template_routine_assignments", (scope) => {
    ...
    app.service(
      ...
      (scope) => {
        return createService({
          programTemplateRoutineAssignmentsRepository: scope.make("repository.program_template_routine_assignments")
        });
      },
      {
    ...
    app.actions(
      withActionDefaults(
        ...
        }),
        {
          domain: "crud",
          dependencies: {
            programTemplateRoutineAssignmentsService: "crud.program_template_routine_assignments"
          }
        }
      )
    );
  ...
  async boot(app) {
    ...
    await addResourceIfMissing(
      api,
      "programTemplateRoutineAssignments",
...
export { ProgramTemplateRoutineAssignmentsProvider };
