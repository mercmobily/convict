...
const CRUD_MODULE_CONFIG = Object.freeze({
  namespace: "workout_sets",
  surface: "app",
  ownershipFilter: "user",
  relativePath: "/workout-sets"
});
...
function resolveCrudPolicyFromApp(app) {
  return resolveCrudSurfacePolicyFromAppConfig(CRUD_MODULE_CONFIG, resolveAppConfig(app), {
    context: "WorkoutSetsProvider"
  });
}

class WorkoutSetsProvider {
  static id = "crud.workout_sets";
  ...
  register(app) {
    ...
    app.singleton("repository.workout_sets", (scope) => {
    ...
    app.service(
      ...
      (scope) => {
        return createService({
          workoutSetsRepository: scope.make("repository.workout_sets")
        });
      },
      {
    ...
    app.actions(
      withActionDefaults(
        ...
        }),
        {
          domain: "crud",
          dependencies: {
            workoutSetsService: "crud.workout_sets"
          }
        }
      )
    );
