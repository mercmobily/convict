...
const CRUD_MODULE_CONFIG = Object.freeze({
  ...
  relativePath: "/program-schedule-entries"
});
...
function resolveCrudPolicyFromApp(app) {
  return resolveCrudSurfacePolicyFromAppConfig(CRUD_MODULE_CONFIG, resolveAppConfig(app), {
    context: "ProgramScheduleEntriesProvider"
  });
}

class ProgramScheduleEntriesProvider {
  static id = "crud.program_schedule_entries";
  ...
  register(app) {
    ...
    app.singleton("repository.program_schedule_entries", (scope) => {
    ...
    app.actions(
      withActionDefaults(
        ...
        }),
        {
          domain: "crud",
          dependencies: {
            programScheduleEntriesService: "crud.program_schedule_entries"
          }
        }
      )
    );
...
export { ProgramScheduleEntriesProvider };
