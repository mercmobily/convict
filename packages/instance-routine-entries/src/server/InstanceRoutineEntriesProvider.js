...
const CRUD_MODULE_CONFIG = Object.freeze({
  ...
  relativePath: "/instance-routine-entries"
});
...
function resolveCrudPolicyFromApp(app) {
  return resolveCrudSurfacePolicyFromAppConfig(CRUD_MODULE_CONFIG, resolveAppConfig(app), {
    context: "InstanceRoutineEntriesProvider"
  });
}

class InstanceRoutineEntriesProvider {
  static id = "crud.instance_routine_entries";
  ...
  register(app) {
    ...
    app.singleton("repository.instance_routine_entries", (scope) => {
    ...
    app.actions(
      withActionDefaults(
        ...
        }),
        {
          domain: "crud",
          dependencies: {
            instanceRoutineEntriesService: "crud.instance_routine_entries"
          }
        }
      )
    );
...
export { InstanceRoutineEntriesProvider };
