...
const CRUD_MODULE_CONFIG = Object.freeze({
  ...
  relativePath: "/workout-occurrences"
});
...
function resolveCrudPolicyFromApp(app) {
  return resolveCrudSurfacePolicyFromAppConfig(CRUD_MODULE_CONFIG, resolveAppConfig(app), {
    context: "WorkoutOccurrencesProvider"
  });
}

class WorkoutOccurrencesProvider {
  static id = "crud.workout_occurrences";
  ...
  register(app) {
    ...
    app.singleton("repository.workout_occurrences", (scope) => {
    ...
    app.actions(
      withActionDefaults(
        ...
        }),
        {
          domain: "crud",
          dependencies: {
            workoutOccurrencesService: "crud.workout_occurrences"
          }
        }
      )
    );
